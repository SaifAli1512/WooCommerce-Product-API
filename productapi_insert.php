<?php  
	...
	try{
		...
		status_message("Products Successfully Imported");
	}
		...
		foreach($json['products'] as $key=>$pd):
		    if(featured_image_present($array_pointer['all_images'][$key])):
    			...
    			if( !$duplicate ){
    				...
    				if($wc_product)
    					status_message('Product added --> ID: '. $wc_product->id."<br>");
    				if(!$debug_mode)
    			...
    			if(!empty($array_pointer['variants'][$key])):
    				...
    				foreach($array_pointer['variants'][$key] as $variant):
    					...
    					if(!$duplicate_variant){
    						...
    						if($vid)
    							status_message('    Variant added --> ID: '. $vid->id."<br>");
    					}
    					else{
    					    unset($temp);
    					    $temp = [	
    			...
    			if(!$debug_mode && !empty($woo_image_ids)):
    			    $pimage['images'] = array();
    				$m = ($old_duplicate)? 1:0;
    				if($old_duplicate):
